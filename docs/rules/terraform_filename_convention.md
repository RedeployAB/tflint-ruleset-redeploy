...
This rule enforces a strict naming convention for Terraform files. Filenames
must follow one of these patterns:

- `<name>.tf`
- `<name>.<area>.tf`

All characters must be lowercase, using snake_case (alphanumeric characters and
underscores). If a filename does not match this pattern, an error is issued.
